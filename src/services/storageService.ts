...
export const initDatabase = async () => {
    ...
    if (count === 0) {
        ...
        const defaultCategories: BudgetCategory[] = [
            { id: 'bc1', name: 'Venta de Servicios', type: 'Ingreso' },
            { id: 'bc2', name: 'Honorarios Contratistas', type: 'Costo Directo' },
            { id: 'bc3', name: 'Gastos Administrativos', type: 'Costo Indirecto' }
        ];
        await supabase.from('budget_categories').insert(defaultCategories);
        ...
        await supabase.from('orders').insert(MOCK_ORDERS.map(o => ({
             ...
             service_details: o.serviceDetails, unit_of_measure: o.unitOfMeasure,
             ...
             commitment_date: o.commitmentDate, client_cert_date: o.clientCertDate,
             billing_date: o.billingDate, history: o.history, attachments: o.attachments,
             progress_logs: [], budget_category_id: o.budgetCategoryId
        })));
    }
};
...
export const saveBudgetEntry = async (entry: Partial<BudgetEntry>) => {
    ...
    await supabase.from('budget_entries').upsert(dbEntry);
};
...
export const saveExchangeRate = async (rate: Partial<ExchangeRate>) => {
    await supabase.from('exchange_rates').upsert(rate);
};
...
export const saveOrder = async (order: Order): Promise<Order[]> => {
    const dbOrder = {
             ...
             budget_category_id: order.budgetCategoryId
    };
...
export const exportBackup = async () => {
    ...
    return JSON.stringify({ orders: o, clients: c, contractors: ct, companies: co, prices: p, users: u, workflow: w, units: un, services: s, timestamp: new Date().toISOString(), version: '1.5-sales-app' }, null, 2);
};
